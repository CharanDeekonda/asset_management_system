...
exports.getAllUsers = async (req, res) => {
    try {
        ...
        const adminEmails = process.env.ADMIN_EMAIL ? process.env.ADMIN_EMAIL.split(',').map(email => email.trim()) : [];
...
exports.createUser = async (req, res) => {
    ...
    try {
        await pool.query('INSERT INTO users (employee_id, name, email) VALUES (?, ?, ?)', [employee_id, name, email]);
...
exports.updateUser = async (req, res) => {
    ...
    const { employee_id, name, email } = req.body;
    try {
        const query = `UPDATE users SET employee_id = ?, name = ?, email = ? WHERE id = ?`;
...
exports.getEmployeeAssetProfile = async (req, res) => {
    ...
    try {
        ...
        const [history] = await pool.query(`
            SELECT a.asset_id, at.name as type, a.brand, a.model, 
                   DATE_FORMAT(ah.from_date, '%d-%m-%Y') as from_date, 
                   DATE_FORMAT(ah.to_date, '%d-%m-%Y') as to_date, ah.remarks
            FROM assignment_history ah
            JOIN assets a ON ah.asset_id = a.asset_id
            JOIN asset_types at ON a.type_id = at.id
            WHERE ah.employee_id = ? AND ah.to_date IS NOT NULL
